...
function getHandTitle(index, hand) {
    ...
    const baseTitle = playerHands.length === 1 ? "Player" : `Hand ${index + 1}`;
...
function getTurnPrompt() {
    if (playerHands.length === 1) {
        return "Your turn: Hit, Stand, or Split if available.";
    }
    return `Playing hand ${activeHandIndex + 1}: Hit or Stand?`;
}
...
function renderHands() {
    ...
    playerHands.forEach((hand, index) => {
        ...
        header.innerHTML = `
          ...
          <div class="score">Bet: ${hand.wager} • Score: ${getHandScore(hand.cards)}</div>
        `;
    ...
    const handLabel = playerHands.length === 1 ? "1 hand" : `${playerHands.length} hands`;
    ...
    if (dealerHidden && !gameOver && dealerHand.length > 0) {
        dealerScoreEl.textContent = `Score: ${getCardValue(dealerHand[0])} + ?`;
    } else {
        dealerScoreEl.textContent = `Score: ${dealerHand.length ? getHandScore(dealerHand) : 0}`;
    }
}
...
function resetSession() {
    ...
    activeHandIndex = 0;
    hasSplit = false;
    statusEl.textContent = "Adjust your bet and deal a round. Try to reach the highest score possible.";
    renderAll();
}
...
function splitHand() {
    ...
    hasSplit = true;
    activeHandIndex = 0;
    statusEl.textContent = "Split placed. Playing hand 1: Hit or Stand?";
    renderAll();
}
...
function playerHit() {
    ...
    if (score > 21) {
        advanceToNextHandOrDealer(playerHands.length > 1 ? `Hand ${activeHandIndex + 1} busted.` : "You busted.");
    } else if (score === 21) {
        advanceToNextHandOrDealer(playerHands.length > 1 ? `Hand ${activeHandIndex + 1} has 21.` : "You have 21.");
    } else {
        statusEl.textContent = getTurnPrompt();
    }
    setControls();
...
function playerStand() {
    if (gameOver) return;
    advanceToNextHandOrDealer(playerHands.length > 1 ? `Hand ${activeHandIndex + 1} stands.` : "You stand.");
    setControls();
}
...
function finishRound(checkImmediateBlackjack = false) {
    ...
    if (checkImmediateBlackjack) {
        ...
        if (playerNatural && dealerNatural) {
            ...
            results.push("Both have Blackjack — push (+0)");
    ...
    if (points <= 0) {
        statusEl.textContent += " You are out of points — reset and try for a better run.";
    }
